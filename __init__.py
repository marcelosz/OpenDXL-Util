...
# McAfee OpenDXL imports
from dxlclient.client import DxlClient
from dxlclient.client_config import DxlClientConfig
from dxlclient.message import Event
# Local settings (global variables)
import settings
...
#
def init(cfg_file):
    settings.opendxl_client = DXLClient(cfg_file)
    if not settings.opendxl_client.connect():
        return False
    return True

def send_status_msg(msg):
    if not settings.opendxl_client:
        return False
    # TODO - check if the client is connected before publishing
    settings.opendxl_client.publish("/opendxl/webhooks/event/status", msg)